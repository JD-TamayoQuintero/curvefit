                  ...
                  *plhs[2])
{
  ...
  emxInit_real_T(&st, &Xpoint, 1, true);
  emxInit_real_T(&st, &Ypoint, 1, true);
  ...
  XYpoints(image, value, Xpoint, Ypoint);

  /* Marshall function outputs */
  Xpoint->canFreeData = false;
  plhs[0] = emlrt_marshallOut(Xpoint);
  emxFree_real_T(&Xpoint);
  emxFree_real_T(&image);
  if (nlhs > 1) {
    Ypoint->canFreeData = false;
    plhs[1] = emlrt_marshallOut(Ypoint);
  }

  emxFree_real_T(&Ypoint);
