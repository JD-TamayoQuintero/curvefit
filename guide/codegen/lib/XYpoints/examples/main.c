...
static void main_XYpoints(void)
{
  ...
  emxInitArray_real_T(&Xpoint, 1);
  emxInitArray_real_T(&Ypoint, 1);

  /* Initialize function 'XYpoints' input arguments. */
  /* Initialize function input argument 'image'. */
  image = c_argInit_UnboundedxUnbounded_r();

  /* Call the entry-point 'XYpoints'. */
  XYpoints(image, argInit_real_T(), Xpoint, Ypoint);
  emxDestroyArray_real_T(Ypoint);
  emxDestroyArray_real_T(Xpoint);
  emxDestroyArray_real_T(image);
}
